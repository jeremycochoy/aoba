# Matchaé — what's done, and what's left
...
- **Brand & strategy** → [`STRATEGY.md`](STRATEGY.md): brand identity (Matchaé), 5 demographic theories (T-A … T-E), 14-day post calendar, voice, palette, measurement plan.
- **Landing page** → live at **https://www.cochoy.fr/matchae/** (GitHub Pages, repo `jeremycochoy/matchae`). Out-of-stock notice + email capture → form posts to `formsubmit.co/jeremy@redstone.ee`. First submission triggers a one-time "Activate" email from Formsubmit you need to click — see *Manual step 1*.
...
- **Instagram signup form pre-filled** → in the Matchaé Chrome (CDP port :9340), the signup page is loaded and the email / password / DOB / name / handle (`matchae.spread`, confirmed available) are already in. See *Manual step 2*.
...
1. Open the Matchaé site in any browser: https://www.cochoy.fr/matchae/
2. Drop your own email in and submit (you'll be redirected to `thanks.html` — that's expected).
3. In Gmail, find the "Activate Formsubmit" email, click the activation link. Done — all future submissions land in your inbox with subject `Matchaé — new stock notification signup`.

## Manual step 2 — finish Instagram signup (≤ 3 min)

The Matchaé Chrome on `:9340` is sitting on the IG signup page with the form filled and the reCAPTCHA "I'm not a robot" gate showing. reCAPTCHA Enterprise resists automation by design.

1. Bring the Matchaé Chrome window to the front (the one that says "Aidez-nous à confirmer que c'est bien vous").
2. Click the **"Je ne suis pas un robot"** checkbox. If it asks you to pick traffic lights, do that.
3. Click **Suivant**.
4. Instagram will email a 6-digit code to `jeremy+matchae@redstone.ee`. Open Gmail, paste the code in the IG page, click confirm.
