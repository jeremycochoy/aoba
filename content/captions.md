# Matchaé — Instagram captions for the first 14 days

Each entry has: post slot, theory tag (per STRATEGY.md), image to use, caption, hashtags, CTA. Hashtags sit at the bottom of the caption (Instagram convention) — 8–15 max, mixing one or two big-volume tags with several mid-tail ones so we're not drowning in over-competed pools.

Captions are written in the brand voice: calm, confident, ingredient-led. No "guilt-free." No exclamation marks (rule of one).

---

## Day 1 — Slot 1 (T-A) — `img/t-a-ingredients.jpg`

**Caption:**
Four ingredients. Read them out loud.

Ceremonial Uji matcha. Organic cocoa butter. Raw wildflower honey. A pinch of sea salt.

That's the whole jar. No palm oil. No refined sugar. No vegetable shortenings hiding behind "natural flavour."

We started Matchaé because the green spreads on the shelf were mostly white chocolate with a wave at matcha. We wanted the opposite — matcha first, sweetness second, everything else nowhere.

First batch is out — link in bio for the next one.

#cleaneating #realfood #noaddedsugar #palmoilfree #matcha #cleanlabel #realingredients #organic

---

## Day 1 — Slot 2 (T-B) — `img/t-b-uji.jpg`

**Caption:**
Uji is where matcha learned to taste like the leaf instead of the powder.

Our matcha is first-harvest, stone-ground, from a single Uji farm. Shaded for the last weeks before picking — which is what builds the amino acids that make ceremonial-grade matcha taste sweet on its own, before any honey gets near it.

A spread is only as good as the ingredient it leads with. We led with this one.

#matcha #ceremonialmatcha #ujimatcha #firstharvest #matchalove #tealover #japanesematcha #greentea

---

## Day 2 — Slot 1 (T-E) — `img/t-e-croissant.jpg`

**Caption:**
Croissant. Warm. Sliced. Half a spoonful of Matchaé in the fold.

The butter does its thing on one side, the cocoa butter does its thing on the other, and somewhere in the middle the honey and the matcha meet. We're still working out if that's a recipe or a religious experience.

Save this one for Sunday morning.

Link in bio for the next batch.

#croissant #matcha #brunch #foodphotography #matcharecipe #f52grams #weekendbreakfast #bakingfromscratch

---

## Day 2 — Slot 2 (T-C) — `img/t-c-toast.jpg`

**Caption:**
The kid test:

"Is it green?"
Yes.
"Is it sweet?"
Yes.
"Can I have more?"
…we'll think about it.

Matchaé is sweetened only with raw honey. The green is matcha, not food dye. The fat is cocoa butter, not palm oil. The "yes I'll have seconds" energy is theirs.

A jar lasts about a week in a house with kids. We've timed it.

Link in bio when the next batch ships.

#momlife #kidsfood #healthysnacks #healthyforkids #nopalmoil #toast #breakfastideas #realfood

---

## Day 3 — Slot 1 (T-D) — `img/t-d-yogurt.jpg`

**Caption:**
Greek yogurt + a spoon of Matchaé.

No protein powder, no maple syrup, no "natural flavourings."

A spoon of this spread carries:
— ceremonial Uji matcha (L-theanine, catechins)
— cocoa butter (a real fat, not a seed oil)
— raw honey (one sweetener, not three)

That's it. Eat the ingredients, not the label promise.

Link in bio — next batch coming.

#realfood #seedoilfree #honeysweetened #wellness #breakfast #greekyogurt #cleaneating #fuelyourbody

---

## Day 3 — Slot 2 (T-A) — `img/t-a-jar-pour.jpg`

**Caption:**
A small honesty post.

Most green spreads on the shelf are this, in order:
sugar → palm oil → cocoa butter → milk powder → emulsifier → matcha (last, often <2%).

Ours is this, in order:
cocoa butter → raw honey → ceremonial matcha → sea salt.

That's the whole list. We could pretend that took marketing genius. It didn't — it just took caring about what we'd actually want to eat.

Link in bio.

#cleanlabel #palmoilfree #realfood #ingredientmatters #matcha #organic #noaddedsugar #cleaneating

---

## Day 4 — Slot 1 (T-B) — `img/t-b-chasen.jpg`

**Caption:**
A chasen has between 80 and 120 prongs of bamboo. It exists because matcha will only give up its taste if you treat it gently.

We've spent a year on the rest of the recipe with the same patience. The cocoa butter we use is unrefined. The honey is single-origin and unfiltered. The salt is a pinch. Nothing in the jar is louder than the matcha — that's the whole point.

#chasen #matcha #ceremonialmatcha #ujimatcha #wabisabi #japanesetea #matchalove #craftsmanship

---

## Day 4 — Slot 2 (T-E) — `img/t-e-cookie.jpg`

**Caption:**
Matcha sandwich cookie, with a generous layer of Matchaé in the middle.

The cookie part is a brown-butter sablé. The middle part is doing all the talking.

(Recipe-curious? Comment "cookie" and we'll DM the printable.)

Link in bio — next batch on the way.

#cookies #matcha #matcharecipe #f52grams #dessertstagram #bakingfromscratch #foodphotography #sandwichcookies

---

## Day 5 — Slot 1 (T-C) — `img/t-c-lunchbox.jpg`

**Caption:**
Lunchbox audit.

Sliced brioche, Matchaé, strawberries, a small handful of blueberries. The brioche is yesterday's. The spread is the green hero. The fruit is the supporting cast.

Three ingredients across the whole box — none of which are hidden seed oils, food dye, or "natural flavour" doing the heavy lifting.

#kidsfood #lunchbox #momlife #healthysnacks #breakfastideas #realfood #nopalmoil #afterschoolsnack

---

## Day 5 — Slot 2 (T-D) — `img/t-d-macros.jpg`

**Caption:**
The label, plainly:

Per tablespoon (~15g):
— Calories: ~95
— Fat: 8g (cocoa butter — not palm, not seed)
— Carbs: 6g (entirely from raw honey)
— Protein: 1g
— Added sugar: 0g

That's the whole macro story. No "proprietary blend," no maltitol, no "natural flavour."

We didn't engineer the macros — we just used real food, and these are the macros real food gives you.

#realfood #honeysweetened #seedoilfree #wellness #macros #cleaneating #fuelyourbody #nutritionfacts

---

## Day 6 — Slot 1 (T-A) — `img/grid-3.jpg`

**Caption:**
The pour test.

Good spread should fall off a spoon in a slow, even ribbon — not a wet drip (too much sugar syrup), not a stiff crumble (too much palm oil hiding the structure).

This is what cocoa butter + raw honey + matcha looks like when it's allowed to behave naturally. No emulsifiers, no stabilisers. Just the spoon and what's on it.

Link in bio for the next batch.

#palmoilfree #realfood #matcha #cleanlabel #cleaneating #noaddedsugar #organic #foodphotography

---

## Day 6 — Slot 2 (T-B) — `img/grid-2.jpg`

**Caption:**
The first Uji harvest of the year is brief — a few weeks in late spring when the youngest leaves are shaded, picked, steamed, and stone-ground.

That's the matcha we use. Not last year's, not culinary-grade, not the dust at the bottom of the bag.

A spread inherits the ingredient at its centre. So we built the recipe around the best matcha we could find, and asked the rest of the jar to keep up.

#ujimatcha #firstharvest #ceremonialmatcha #matcha #japanesematcha #tealover #matchalove #craftsmanship

---

## Day 7 — Slot 1 (T-E) — `img/t-e-swirl.jpg`

**Caption:**
Two spoons of Matchaé folded into vanilla cheesecake batter, swirled (not mixed) before it bakes.

You get a green ribbon through every slice, a hit of matcha that comes and goes, and a sweetness that's coming from honey, not sugar.

Tag the friend whose Sunday this is becoming.

Link in bio when the next batch ships.

#matcha #cheesecake #dessertstagram #foodphotography #bakingfromscratch #f52grams #matcharecipe #weekendbaking

---

## Day 7 — Slot 2 (T-C) — `img/t-c-toast.jpg` (Reel — kid reaction)

**Caption (Reel):**
The first-bite face is honest. Every time. Green spread, raw honey, real matcha — apparently exactly what a 7-year-old wants on a Saturday.

(Sound: kitchen ambient + a small "mmm".)

Link in bio for the next batch.

#momlife #kidsfood #toast #healthysnacks #healthyforkids #reelsoftheday #realfood #nopalmoil

---

(Continued for days 8–14 in `captions-week2.md`; the same shape and theory rotation continues so we hit ≈3 posts per theory by day 14.)
